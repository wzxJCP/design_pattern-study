...
/**
 * 单一职责原则（SRP）的核心一句话总结：
 * 一个类应该只有一个引起它变化的原因，即每个类只负责一项独立的职责。
 * 结合代码理解：
 * 就像建筑材料代码一样，Brick 只负责承重，Rebar 只负责抗拉，Cement 只负责粘结。如果未来需要修改“承重”的逻辑（比如换了一种砖），
 * 只需要修改 Brick 类，而完全不会影响到 Rebar 或 Cement 类。这就是职责单一带来的高内聚、低耦合。
 */