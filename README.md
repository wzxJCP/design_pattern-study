...
* 假设有一个`Bird`类，它有飞行的能力。如果我们想要创建一个新类`FlyingSquirrel`，虽然松鼠不是鸟，但是我们希望它也能飞行。如果使用继承的方式，可能会导致类的设计变得混乱，因为松鼠和鸟类在生物学上差别很大。而使用组合的方式，我们可以定义一个`Flight`接口或者抽象类，然后让`Bird`和`FlyingSquirrel`都包含一个实现了`Flight`接口的对象，这样既保持了各自的特点，又达到了复用飞行能力的目的。
