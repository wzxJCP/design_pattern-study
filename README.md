...
- **OCP（开闭原则）**：是房子的**最终设计蓝图**——房子要能方便地加装电梯、改造房间，但不用拆承重墙。
- **SRP（单一职责）**：是**砖块和钢筋**——每块砖只负责承重，每根钢筋只负责抗拉，职责清晰。
- **DIP（依赖倒置）**：是**标准的接口规范**——所有门窗、水管都按统一标准尺寸制造，方便替换。
- **LSP（里氏替换）**：是**质量检验**——任何品牌的螺丝钉，只要规格一致，就能拧进去，不会滑丝。
- **ISP（接口隔离）**：是**专用工具**——螺丝刀只负责拧螺丝，锤子只负责敲钉子，不搞万能工具。
- **LoD（迪米特法则）**：是**施工规范**——水电工只和墙里的管道打交道，不和木工的工具箱直接接触。
- **CRP（合成复用）**：是**模块化装修**——用组合柜（组合）代替整体浇筑的柜子（继承），想换风格随时拆装。
