...
1. **OCP（开闭）**：是房子的**最终设计蓝图**——房子要能方便地加装电梯、改造房间，但不用拆承重墙；
2. **SRP（单一职责）**：是**砖块和钢筋**——每块砖只负责承重，每根钢筋只负责抗拉，职责清晰；
3. **DIP（依赖倒置）**：是**标准的接口规范**——所有门窗、水管都按统一标准尺寸制造，方便替换；
4. **LSP（里氏替换）**：是**质量检验**——任何品牌的螺丝钉，只要规格一致，就能拧进去，不会滑丝；
5. **ISP（接口隔离）**：是**专用工具**——螺丝刀只负责拧螺丝，锤子只负责敲钉子，不搞万能工具；
6. **LoDP（迪米特）**：是**施工规范**——水电工只和墙里的管道打交道，不和木工的工具箱直接接触；
7. **CRP（合成复用）**：是**模块化装修**——用组合柜（组合）代替整体浇筑的柜子（继承），想换风格随时拆装。

**结论：**

没有绝对的顺序，**但OCP是目标、SRP是基础、DIP是核心手段；LSP是保障；ISP、LoD、CRP是辅助和优化。**

在实际编码中，**SRP（单一职责）** 和 **DIP（依赖倒置）** 是最先、最频繁用到的两个原则，它们是写出好代码的起点。
...
**开闭原则（OCP）的核心一句话总结：**软件实体（类、模块、函数等）应该对扩展开放，对修改关闭。
...
**单一职责原则（SRP）的核心一句话总结：**一个类应该只有一个引起它变化的原因，即每个类只负责一项独立的职责。
...
**依赖倒置原则（DIP）的核心一句话总结：**高层模块不应依赖低层模块，二者都应依赖抽象；抽象不应依赖细节，细节应依赖抽象。
...
**里氏替换原则（LSP）的核心一句话总结：**子类必须能够替换父类，且替换后程序的行为逻辑依然正确，不会破坏父类定义的行为契约。
...
**接口隔离原则（ISP）的核心一句话总结：**客户端不应被迫依赖于它不需要的接口，应将臃肿的“万能接口”拆分为更小、更具体的“专用接口”。
...
**迪米特原则（LoD）的核心一句话总结：**一个对象应该对其他对象有最少的了解，只和“直接朋友”通信，不要和“陌生人”说话。
...
**合成复用原则（CRP）的核心一句话总结：**优先使用组合或聚合（“拥有”关系）来复用代码，而不是继承（“是”关系），以保持系统的灵活性和低耦合。
...
除GOF23种外的设计模式：还有简单工 厂模式，由一个工厂决定创建哪一种产品类型的实例。虽然被广泛使用，但因为它**违背了开闭原则**（新增产品需要修改工厂类），GoF的作者们没有把它列入23种。
